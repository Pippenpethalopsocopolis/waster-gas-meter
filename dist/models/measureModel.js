...
export const addMeasure = async (measure, image) => {
    ...
    const [rows] = await connection.execute('SELECT * FROM measures WHERE customer_code = ? AND measure_type = ? AND measure_datetime = ?', [measure.customer_code, measure.measure_type, newMeasureDate]);
    if (rows.length > 0) {
        connection.release();
...
export const findMeasureByUuid = async (measure_uuid) => {
    const connection = await connectionPool.getConnection();
    const [rows] = await connection.execute('SELECT * FROM measures WHERE measure_uuid = ?', [measure_uuid]);
    connection.release();
    const row = rows[0]; // Cast to any or a more specific type if known
    if (!row) {
        return undefined;
    }
    // Access row data
...
export const findMeasuresByCustomerCode = async (customer_code, measure_type) => {
    const connection = await connectionPool.getConnection();
...
export const confirmMeasure = async (measure_uuid, confirmed_value) => {
